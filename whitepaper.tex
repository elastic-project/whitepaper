...
The workers (here, miners) then return the output of the arbitrary task, e.g., $[y_1,\ldots,y_l] = T(xi,\ldots,x_k)$ along with a proof-of-work that the computation of $T$ was carried out correctly.
...
As, in our scheme, users are rewarded for contributing computational power, this proof-of-work must be secure. Otherwise, it can be assumed that dishonest users will find ways to return plausible results without performing any actual work. To solve this problem, we rely on a method that has been introduced by \cite{ben2013snarks}. More precisely, we rely on zkSNARK (zer-knowledge Succinct Non-interactive ARgument of Knowledge) schemes \cite{bitansky2013succinct,bitansky2012extractable,ben2014succinct}, which are a cryptographic method of verifying the integrity of arbitrary computations. \textbf{NOTE: Not sure if we should write how that works here or just refer to the paper?}.

This does not solve the mid-execution feedbacks, that allow for a more fine-granulated payout of rewards yet. That is, we also want to payout \emph{weaker} miners that not necessarily manage to calculate the arbitrary task until the end (or termination) before others have already consumed all $R$ reward coins. To mitigate this problem we need a way for everyone to constantly and periodically provide proof-of-works for sub-portions of the arbitrary task.
We suggest to adapt the scheme described above as follows:
